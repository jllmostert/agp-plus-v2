...
export async function addSensor(sensorData) {
  ...
  if (existingIndex === -1) {
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify(db));
    console.log('[addSensor] New sensor added:', sensorData.id);
  } else {
    // Sensor exists - update if newly detected as active
    const existingSensor = db.sensors[existingIndex];
    const isReactivated = 
    ...
    if (isReactivated) {
      console.log('[addSensor] Sensor re-detected as active, updating status:', sensorData.id);
      
      // Update to active status
      existingSensor.lifecycle = 'active';
      existingSensor.status = 'running';
      existingSensor.success = null;  // ← FIX: Reset success field
      existingSensor.end_date = null;
      existingSensor.duration_hours = null;
      existingSensor.duration_days = null;
      existingSensor.reason_stop = null;
      
      db.lastUpdated = new Date().toISOString();
      localStorage.setItem(STORAGE_KEY, JSON.stringify(db));
    } else {
      console.log('[addSensor] Sensor already exists, no update needed:', sensorData.id);
    }
  }
}
